...
instance (CRemove x xs out) => CRemove' 'False x (y ': xs) (y ': out) where
  remove' _ p (HSortedList (L.HCons y tail')) = HSortedList $ L.HCons y $ unwrap
    (remove p (HSortedList tail')) & trace "rec"
...
instance (CInsert (CmpToSymbol x z) x (z ': xs) res)  =>  CInsert 'GT x (y ': z ': xs) (y ': res) where
  insert' _ x' (HSortedList (L.HCons y xs)) = HSortedList $ L.hcons
    y
